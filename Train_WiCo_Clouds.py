...
def main():        
    net = Net(3, num_classes=MY.num_classes+1, size_context=args['size_context'], size_local=args['size_local']).cuda()
        
    train_set = MY.Loader(args['data_dir'], 'train', random_crop=True, crop_nums=args['crop_nums'], random_flip=True,
    ...
    val_set = MY.Loader(args['data_dir'], 'val', sliding_crop=True, size_context=args['size_context'], size_local=args['size_local'])
...
def validate(val_loader, net, criterion, curr_epoch):
    ...
    for vi, data in enumerate(val_loader):
        ...
        if args['save_pred'] and vi==0:
            pred_color = MY.Index2Color(preds[0])
